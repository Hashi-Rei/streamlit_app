...
df_base = df[
    (df['都道府県名'].isin(Prefs)) &
    (df['元号'].isin(Gengou)) & 
    (df['西暦（年）'] >= year[0]) & (df['西暦（年）'] <= year[1])
]
#⇑「指定した左の年より前」と「指定した右の年より後」のデータを全部捨てて、その間のデータだけを残す という命令
# （year[0]が開始年、year[1]が終了年）


# 折れ線グラフ専用（ユーザーがどの年齢区分を選んでいても、総数を必ず使う）
df_line = df_base[df_base['年齢5歳階級'] == '総数'].sort_values('西暦（年）')
...
if Prefs:
    ...
    if not df_line.empty:
        ...
        st.warning("表示するデータがありません。サイドバーで都道府県・元号を選んでください。",)
    ...
    # 最新の1年分だけ抜き出す。(これをしないと、選択した全年代が足し算されて巨大な分かりづらい棒になってしまう。)
        ...
        if not df_bar.empty:
            ...
            st.warning("年齢層を選択してください。(※総数は反映されません。)")
    ...
    # 何も選んでいない時だけ説明を表示する
    with st.expander("データの抽出の仕方"):
        st.success('左側のサイドバーから都道府県を選択すると、自動的に分析が開始されます。')
        st.write("※総数を選択しても棒グラフは影響しません。\n期間のサイドバーを動かすと、同時に棒グラフが変化します。")


# 結果
st.subheader(f'抽出データ({len(df_display)}件)')
